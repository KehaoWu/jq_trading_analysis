...
def plot_hedge_results(hedge_data_file, output_file=None):
    ...
    # 计算累积收益率（以1为基准）
    hedge_cumulative = (1 + df['hedge_return'] / 100).cumprod().tolist()
    backtest_cumulative = (1 + df['backtest_return'] / 100).cumprod().tolist()
    index_cumulative = (1 + df['index_return'] / 100).cumprod().tolist()
    ...
    cumulative_chart = (
        ...
        .add_yaxis(
            series_name="对冲组合累积收益率",
            y_axis=hedge_cumulative,
            label_opts=opts.LabelOpts(is_show=False),
            linestyle_opts=opts.LineStyleOpts(width=2),
            itemstyle_opts=opts.ItemStyleOpts(color="#1890ff"),
        )
        .add_yaxis(
            series_name="回测策略累积收益率",
            y_axis=backtest_cumulative,
            label_opts=opts.LabelOpts(is_show=False),
            linestyle_opts=opts.LineStyleOpts(width=2),
        ...
        .add_yaxis(
            series_name="指数累积收益率",
            y_axis=index_cumulative,
            label_opts=opts.LabelOpts(is_show=False),
            linestyle_opts=opts.LineStyleOpts(width=2),
            itemstyle_opts=opts.ItemStyleOpts(color="#52c41a"),
        )
        .set_global_opts(
            title_opts=opts.TitleOpts(title="累积收益率对比"),
...
def analyze_hedge_performance(hedge_data_file):
    ...
    # 计算累积收益率
    hedge_cumulative_return = (1 + df['hedge_return'] / 100).cumprod().iloc[-1] - 1
    hedge_annual_return = (1 + hedge_cumulative_return) ** (1/years) - 1
    ...
    # 计算对冲组合的累积收益率序列用于回撤计算
    hedge_cumulative_series = (1 + df['hedge_return'] / 100).cumprod()
    hedge_max_drawdown = calculate_max_drawdown(hedge_cumulative_series)
